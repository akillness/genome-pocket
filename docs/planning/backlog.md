...
- [~] **POCKET-404: LLM & Entity-Resolution Ops** *(404a + offline ops + 404b + 404c delivered; 404d in progress — see `docs/architecture/graph-target.md`)*
  ...
  - *Remaining:* **404d** fuse graph hits into the RRF retriever as a third list (`mode="graph"`/`"hybrid"`) + MCP `traverse_graph`; HITL approval for low-confidence facts folds into POCKET-302.
