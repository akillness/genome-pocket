...
leaf chunks (1000자)
    └─ summarized cluster (≈2000자 요약, LLM or extractive)
        └─ root summary (전체 파일 요약)
...
query
  ├─ query variant 1 ("... 다르게 표현한 버전")
  ├─ query variant 2
  └─ query variant 3
       → 각각 독립 retrieve → 4-list RRF merge
...
임팩트 ↑
  │  [리랭커 추가]       [GraphRAG 커뮤니티]
  │  [HyDE 쿼리 확장]   [RAPTOR 계층 청킹]
  │  [Qwen3-8B 업그레이드]
  │  [Late Chunking]    [RAGAS 평가]
  │  [LanceDB 전환]     [SPLADE BM42]
  └────────────────────────────────── 구현 난이도 →
       낮음                               높음
