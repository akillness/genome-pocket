...
| # | Problem | Fix |
|---|---------|-----|
| T1 | `SentenceTransformerEmbedder` downloads model on first import | Add `MockEmbedder` returning deterministic fixed-dim zeros |
| T2 | No unit tests for `pocketindex/` internals | Add `tests/test_engine.py` |
| T3 | No fingerprint integration tests | Add fingerprint round-trip tests |
| T4 | Graph tests require a real pipeline run | Add `tests/test_graph_unit.py` with `DeterministicExtractor` + in-memory SQLite |
| T5 | `pytest` not in dev dependencies | Added via `uv add --dev pytest`; needs `uv.lock` commit |
