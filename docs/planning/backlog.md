...
- [x] **POCKET-301: Local Tracing & Lineage UI** *(done — web UI + interactive graph-review slice)*
  ...
  - *Delivered (web-UI surface):* `pocket serve` now serves a single, dependency-free HTML page at `GET /` (`pocket/web_ui.py`) that visualizes **how a query was routed** and **which source files answered it**. The testable core is `retrieval.routing_trace()`, which reuses the same `_gather`/`_fuse` orchestration as `search()` (extracted into a shared `_gather` helper + a `_MODE_STRATEGIES` routing table) and reports, per strategy (`vector`/`lexical`/`graph`), whether the chosen mode *activates* it, whether it is *available* on this target (FTS / graph tables present), and its candidate count — plus the fused hits, each tagged with the `contributors` that surfaced it. Exposed over HTTP as `GET /trace`; the page lazy-loads per-file chunk lineage via the existing `/lineage` endpoint. Chose the existing Starlette server + vanilla JS over Streamlit/React to honor Pocket's local-first, no-heavy-deps DNA. Tests: trace strategy/contributor annotation, lexical-mode routing isolation, missing-index empty trace, and the `/` + `/trace` endpoints (4).
...
- [x] **POCKET-303: Automated Retrieval Evaluation** *(done)*
  ...
  - *Delivered:* `pocket/evaluation.py` + a `pocket eval` command make retrieval quality a measurable, CI-gateable number. It computes standard IR metrics — **Hit@k, MRR, Precision/Recall@k, MAP** (pure scoring functions with lenient exact/suffix/basename path matching) — over either a hand-written gold set (`--cases <json>`, `{query, relevant_files[, mode]}`, validated on load) or **synthetic query/context pairs mined from the index** (`synthesize_cases()`): for each source file it selects the tokens most *distinctive* to it (lowest cross-file document frequency, then length), joins them into a self-labeled query whose only correct answer is that file, so no curated gold set is needed and any indexing/chunking regression surfaces as a dropped hit. `evaluate()` runs every case through the real `retrieval.search()` (per-case `mode`), so the harness can never diverge from production retrieval. `save_baseline()`/`load_baseline()` persist a run's aggregates and `compare_to_baseline()` (with `--tolerance`) fails `pocket eval --baseline <json>` (exit 1) on any metric below the baseline; `--save` records a new one. Synthetic cases default to `lexical` (deterministic distinctive-token probes independent of the installed model); `--mode hybrid/vector` evaluates the semantic path. Tests: metric primitives + path matching, synthetic self-retrieval (perfect Hit@k/MRR), empty-index safety, gold-case parsing + validation errors, baseline round-trip + tolerance-aware regression, CLI end-to-end (6).
  - *Remaining:* none.
