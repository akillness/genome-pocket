...
- [ ] **POCKET-404: LLM & Entity-Resolution Ops** *(spec'd — see `docs/architecture/graph-target.md`)*
  - *Gap:* upstream offers `ops.litellm` (LLM extraction) and `ops.entity_resolution` (faiss-backed dedup); pocketindex has neither, and both need a graph target that did not exist.
  - *Unblocked:* graph target now designed in `docs/architecture/graph-target.md` — a local-first GraphRAG layer (SQLite-resident `entities`/`relations` reusing the existing lineage/memo/sweep machinery, optional Ollama/LiteLLM extraction, sqlite-vec blocked + LLM-adjudicated entity resolution). Spec grounded in a 2025–2026 arXiv survey (MemGraphRAG, FlowRAG, schema-agnostic + uncertainty-guided KG construction, cost-effective LLM entity resolution).
  - *Split into:* **404a** graph schema/target (offline-testable, no LLM — first slice), **404b** LLM extraction op, **404c** entity-resolution op, **404d** GraphRAG retrieval fusion; HITL approval for low-confidence facts folds into POCKET-302.
