...
[ RecursiveSplitter ] ──> Generates Chunks (text, start/end offsets)
      │
      ▼ (process_chunk)
...
The `process_file` function is decorated with `@coco.fn(memo=True)`. When a file is scanned, CocoIndex computes a hash of the file content and the function's code. If the hash matches a previously processed state, the function is skipped, avoiding redundant chunking and embedding.
